...
import entity.dto.InterfaceEntityDTO;

import java.io.IOException;

public class InterfaceEntityDTOAdapter extends TypeAdapter<InterfaceEntityDTO> {

  private final AdditionalBinDTOAdapter additionalBinDTOAdapter;
  private final LocationDTOAdapter locationDTOAdapter;

  public InterfaceEntityDTOAdapter() {
    this.additionalBinDTOAdapter = new AdditionalBinDTOAdapter();
    this.locationDTOAdapter = new LocationDTOAdapter();
  }

  @Override
  public void write(JsonWriter out, InterfaceEntityDTO value) throws IOException {
    out.beginObject();
  ...
  @Override
  public InterfaceEntityDTO read(JsonReader in) throws IOException {
    InterfaceEntityDTO res = new InterfaceEntityDTO();
    in.beginObject();
    while (in.hasNext()) {
      switch (in.nextName()) {
        case "external":
